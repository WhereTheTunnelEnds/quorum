...
| An adapter that asks the MODEL to fill envelope fields gets invented values | field-notes, claude-alt-agent | dispatch through the agent twice and compare `is_error` in the envelope against `jq .is_error` on the same call's raw JSON |
