...
| A second Claude subscription reports `is_error:true` while `subtype` still says `success` | field-notes, claude-alt-agent | `CLAUDE_CODE_OAUTH_TOKEN=invalid claude -p 'hi' --output-format json \| jq '{is_error,subtype}'` |
| Headless `claude -p` DENIES writes at the harness, and records it in `.permission_denials` | claude-alt-agent, safety-model | ask it to edit a file in a scratch repo, then `jq '.permission_denials' ` — expect one entry naming `Edit`, and the file unchanged |
| `--dangerously-skip-permissions` lifts that boundary | claude-alt-agent | same request with the flag — the file is written |
| `jq '.is_error // true'` rewrites a successful `false` to `true` | field-notes, claude-alt-agent | `echo '{"is_error": false}' \| jq -r '.is_error // true'` — prints `true` |
| `quorum-verify` fails when a good call matches the probe's own BROKEN_MATCH | field-notes, quorum-verify | reintroduce `// true` in `probes/claude-alt.sh` and run `quorum-verify claude-alt` — expect a red reachability line naming BROKEN_MATCH |
| `plugin update` compares version strings, not file contents | field-notes | edit a shipped file without bumping the version, run `claude plugin update quorum`, then `tests/test-deployed-matches-repo.sh` — expect "already at the latest version" and a red gate |
| The deployed copy is resolved from `installed_plugins.json`, not by mtime | field-notes, test-deployed-matches-repo | `jq -r '(.plugins//{})\|to_entries[]\|select(.key\|test("^quorum@"))\|.value[]?\|.installPath' ~/.claude/plugins/installed_plugins.json` and compare with `ls -dt ~/.claude/plugins/cache/*/quorum/*/ \| head -1` |
...
|---|---|---|
| Four independently-written implementations of one brief normalise to four DIFFERENT hashes — structurally divergent, yet unanimous in the flaw that mattered | capability spec §7, `field-notes.md` | The four files lived in throwaway worktrees that have been deleted, and model outputs are not deterministic, so the exact four cannot be regenerated. The *method* is repeatable: dispatch one brief to four providers, then hash each result with `sed 's/[[:space:]]\+/ /g' FILE \| grep -v '^#' \| sort \| md5`. |
